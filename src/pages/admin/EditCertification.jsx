...
import { updateCertification } from "../../utils/certificationAdminApi";
  ...
  useEffect(() => {
    const fetchCertification = async () => {
      ...
      try {
        ...
        const snapshot = await getDoc(doc(dbFirestore, "certifications", id));
        ...
        setLocalizedFormData({
          title: getIdText(data.title),
          issuer: getIdText(data.issuer),
          category: getIdText(data.category),
          alt: getIdText(data.alt),
        });
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        order: Number(formData.order) > 0 ? Number(formData.order) : 0,
        isPublished: Boolean(formData.isPublished),
        img: imageUrl,
        updatedAt: new Date(),
      };

      await updateCertification({ auth, id, payload });

      setSuccess("Certification updated successfully!");
      showToast({
        type: "success",
        title: "Certification updated",
        message: "Certification changes were saved.",
      });
      ...
      if (err?.code === "permission-denied") {
        setError(
          "Failed to update certification: Firestore blocked write access (permission-denied). Check Firestore Rules for `certifications` and ensure your account is an admin."
        );
        showToast({
          type: "error",
          title: "Update failed",
        ...
        setError("Failed to update certification: your session has expired. Please login again.");
        showToast({
          type: "error",
          title: "Session expired",
          message: "Please login again before updating certification.",
        });
      } else {
        setError(`Failed to update certification: ${err?.message || String(err)}`);
        showToast({
          type: "error",
          title: "Update failed",
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-dark px-4 md:px-8 pt-24 pb-10">
        <div className="max-w-4xl mx-auto bg-white dark:bg-slate-800 p-10 rounded-2xl border border-gray-200 dark:border-slate-700 text-center">
          <p className="text-gray-600 dark:text-gray-300">Loading certification data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark px-4 md:px-8 pt-24 pb-10">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-3xl md:text-4xl font-black text-dark dark:text-white">Edit Certification</h1>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-200 dark:border-slate-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div>
                <label className="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-300">Display Order</label>
                <input
                  type="number"
                  min="0"
                  name="order"
            ...
            {formData.currentImage && (
              <div className="mb-3">
                ...
                <img src={formData.currentImage} alt="Current" className="w-40 h-24 object-cover rounded border" />
              </div>
            )}
            ...
            {imagePreview && (
              <div className="mb-3">
                ...
                <img src={imagePreview} alt="Preview" className="w-56 h-32 object-cover rounded border" />
              </div>
            )}
